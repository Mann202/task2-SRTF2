...
for i in range(n):  # Chạy vòng lặp
    ...
    Process[i] = [BurstTime[i], ArrivalTime[i], i, StartTime[i]]  # Cho BurstTime, ArrivalTimes và thứ tự Process vào một mảng Process
...
print(list)
for i in list:
    finishTime[i[0][2]] = i[1] # Thêm thời gian hoàn thành (Finish time) vào mảng finishTime theo thứ tự i của Process4
...
for i in range(len(finishTime)):    # Chạy vòng lặp với độ dài của mảng Finish Time
	print("{}\t{}\t\t{}\t\t{}\t\t{}\t\t{}\t\t{}\t\t{}\n".format(i, BurstTime[i], ArrivalTime[i],StartTime[i], finishTime[i], resTime[i], turnTime[i], waitTime[i]))   #In ra màn hình kết quả
